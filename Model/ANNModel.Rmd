...
#index <- sample(1:nrow(data),round(0.75*nrow(data))) Want to assess the same data!
train <- data[index,]
test <- data[-index,]
lm.fit <- glm(efficiency~., data=train) # fitting reg linear regression model
#summary(lm.fit)
pr.lm <- predict(lm.fit,test)
RMSE.lm <- sum((pr.lm - test$efficiency)^2)/nrow(test) # MSE is mean squared error
...
test.r <- (test_$efficiency)*(max(data$efficiency)-min(data$efficiency))+min(data$efficiency)
RMSE.nn <- sum((test.r - pr.nn_)^2)/nrow(test_)
